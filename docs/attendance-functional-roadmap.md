...
| 4 | Correction requests and second-level approval | Gives supervisors and HR a controlled way to correct locked attendance with evidence, comments, ownership, and audit history. | High | Approval workflow, attachments, role policy, payroll-lock integration | Delayed payroll if approval ownership is unclear | Phase 2B |
