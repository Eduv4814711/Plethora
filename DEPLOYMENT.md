...
3. Open the API service **Settings**:
   ...
   - **Build Command**: `npm run build` (runs `prisma generate`, `tsc`, and template copy)
   - **Start Command**: `npm start` (runs `prisma migrate deploy` then `node dist/index.js`)
...
- **Web API calls go to the wrong host**: `NEXT_PUBLIC_API_URL` is baked in at **build** time. Set it on the web service, then redeploy the web app.
