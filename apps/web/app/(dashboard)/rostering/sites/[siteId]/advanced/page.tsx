...
export default function AdvancedRosterEditorPage() {
  ...
  if (loading || !status || !period) return <div className="space-y-4 animate-pulse"><div className="h-24 rounded-2xl bg-security-navy-100 dark:bg-security-navy-800" /><div className="h-96 rounded-2xl bg-security-navy-100 dark:bg-security-navy-800" /></div>;
  ...
  return (
    <div className="space-y-4 animate-fade-in">
      <header className="sticky top-0 z-30 rounded-2xl border border-security-navy-100 bg-white/95 p-4 shadow-security-card backdrop-blur dark:border-security-navy-700 dark:bg-security-navy-900/95">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div><Link href={`/rostering/sites/${siteId}`} className="text-sm font-medium text-security-amber-700 hover:underline dark:text-security-amber-300">← Back to simple roster view</Link><div className="mt-2 flex flex-wrap items-center gap-2"><h1 className="text-xl font-bold text-security-navy-900 dark:text-white">{ongoingMode ? "Change the ongoing schedule" : "Advanced roster editor"}</h1><span className="rounded-full bg-security-navy-50 px-2 py-0.5 text-xs font-medium text-security-navy-600 dark:bg-security-navy-800 dark:text-security-navy-300">Edit access</span></div><p className="mt-1 text-sm text-security-navy-500">{status.siteName} · {ongoingMode ? "Create a new effective-dated repeating schedule without changing roster history." : "Use for bulk edits, patterns, manual sites, and PDF exports."}</p></div>
          <div className="flex flex-col gap-2 sm:flex-row sm:items-end"><label><span className="block text-xs font-medium text-security-navy-500">{ongoingMode ? "Effective roster period" : "Roster period"}</span><select value={period.periodKey} onChange={(event) => { const next = periods.find((item) => item.periodKey === event.target.value); if (next) void changePeriod(next); }} className="input-modern mt-1 w-full sm:w-64">{periods.map((item) => <option key={item.periodKey} value={item.periodKey}>{item.rosterLabel}</option>)}</select></label>{draft.hasUnsavedChanges && <button type="button" onClick={() => void workspaceRef.current?.discardChanges()} disabled={draft.isSaving} className="btn-secondary">Discard</button>}{ongoingMode ? <button type="button" onClick={() => void applyOngoingSchedule()} disabled={!draft.hasUnsavedChanges || draft.hardIssueCount > 0 || draft.isSaving} className="btn-primary disabled:opacity-50">{draft.isSaving ? "Applying…" : "Review and start schedule"}</button> : <button type="button" onClick={() => void workspaceRef.current?.saveRoster()} disabled={!draft.hasUnsavedChanges || draft.isSaving} className="btn-primary disabled:opacity-50">{draft.isSaving ? "Saving…" : `Save ${draft.pendingCount || ""} change${draft.pendingCount === 1 ? "" : "s"}`}</button>}{canApprove && !continuous && !ongoingMode && <button type="button" onClick={() => void workspaceRef.current?.publishRoster()} disabled={!draft.canPublish || draft.isPublishing} className="btn-secondary disabled:opacity-50">{draft.isPublishing ? "Publishing…" : "Publish shifts"}</button>}</div>
        </div>
      ...
      {ongoingMode && <ol className="grid gap-2 sm:grid-cols-4" aria-label="Ongoing schedule workflow"><li className="rounded-security-lg border border-security-amber-300 bg-security-amber-50 p-3 text-sm dark:border-security-amber-800 dark:bg-security-amber-950/30"><span className="font-bold">1.</span> Choose effective period</li><li className="rounded-security-lg border border-security-amber-300 bg-security-amber-50 p-3 text-sm dark:border-security-amber-800 dark:bg-security-amber-950/30"><span className="font-bold">2.</span> Edit repeating shifts</li><li className={`rounded-security-lg border p-3 text-sm ${draft.hardIssueCount > 0 ? "border-red-300 bg-red-50 text-red-800 dark:border-red-800 dark:bg-red-950/30 dark:text-red-200" : "border-security-navy-100 bg-white dark:border-security-navy-700 dark:bg-security-navy-900"}`}><span className="font-bold">3.</span> {draft.hardIssueCount > 0 ? `Fix ${draft.hardIssueCount} issue${draft.hardIssueCount === 1 ? "" : "s"}` : "Check coverage"}</li><li className="rounded-security-lg border border-security-navy-100 bg-white p-3 text-sm dark:border-security-navy-700 dark:bg-security-navy-900"><span className="font-bold">4.</span> Review and start</li></ol>}

      <details open={ongoingMode} className="rounded-2xl border border-security-navy-100 bg-white p-4 shadow-security-card dark:border-security-navy-700 dark:bg-security-navy-900">
        <summary className="cursor-pointer font-semibold text-security-navy-900 dark:text-white">Pattern and team tools</summary>
        <p className="mt-1 text-sm text-security-navy-500">Apply a repeating schedule to one guard or stagger it across the whole team.</p>
        <div className="mt-4">{patternContext ? <GuardPatternBuilder cycleLengthDays={cycleLength} onCycleLengthChange={setCycleLength} rows={patternContext.rows} availableGuards={patternContext.availableGuards} addingGuard={patternContext.addingGuard} onAddGuardToSite={(guardId) => void workspaceRef.current?.addGuardToSite(guardId)} onApply={(guardId, codes) => workspaceRef.current?.applyGuardPattern(guardId, codes as RosterShiftCode[])} onApplyAll={(codes) => workspaceRef.current?.applyPatternToAll(codes as RosterShiftCode[])} /> : <p className="text-sm text-security-navy-500">Loading pattern tools…</p>}</div>
      </details>

      <section className="rounded-2xl border border-security-navy-100 bg-white p-4 shadow-security-card dark:border-security-navy-700 dark:bg-security-navy-900">
