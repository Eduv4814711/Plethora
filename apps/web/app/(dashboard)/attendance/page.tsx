...
function TimesheetUploadSection({
  token,
  employees,
  sites,
  onSuccess,
  ...
  const [extractResult, setExtractResult] = useState<TimesheetExtractResult | null>(null);
  ...
  const handleFile = async (file: File) => {
    if (!file.type.match(/^image\/(png|jpeg|jpg|tiff)$/)) {
    ...
    setPostId("");
    try {
      const result = await extractTimesheetFromImage(token, file);
      setExtractResult(result);
      setEntries(result.extracted.entries ?? []);
      if (result.matchedEmployee) setEmployeeId(result.matchedEmployee.id);
      if (result.matchedSite) {
        setSiteId(result.matchedSite.id);
        if (result.matchedSite.posts.length === 1) setPostId(result.matchedSite.posts[0].id);
      }
    } catch (err) {
  ...
  const handleImport = async () => {
    if (!employeeId || !postId || entries.length === 0) {
      setError("Select employee, post, and ensure there are entries to import.");
      return;
    }
    setError("");
    setImporting(true);
    try {
      ...
      const result = await importTimesheetEntries(token, {
        employeeId,
        postId,
        entries: importEntries,
      });
      if (result.errors.length > 0 && result.created === 0) {
        setError(result.errors[0]?.message ?? "Import failed");
      } else {
        setExtractResult(null);
        setEntries([]);
        setEmployeeId("");
        ...
        if (result.errors.length > 0) {
          setSuccessMessage(`Imported ${result.created} entries. ${result.errors.length} failed.`);
        } else {
          setSuccessMessage(`Successfully imported ${result.created} entries.`);
        }
  ...
  return (
    <div className="card-wireframe mb-6 p-4">
      <h3 className="font-medium text-neutral-800 dark:text-neutral-200 mb-2">
        Import from timesheet image
      </h3>
      <p className="text-sm text-neutral-600 dark:text-neutral-400 mb-4">
        Upload a photo or scan of a paper timesheet. The system will extract attendance data for you to review.
      </p>
      ...
      {!extractResult ? (
        <div
          ...
          className={clsx(
            ...
            dragOver ? "border-neutral-400 bg-neutral-50 dark:bg-neutral-800/50" : "border-neutral-300 dark:border-neutral-600 hover:border-neutral-400 dark:hover:border-neutral-500",
        ...
        >
          ...
          />
          {loading ? (
            <p className="text-neutral-600 dark:text-neutral-400">Extracting data...</p>
          ) : (
            <>
              <p className="text-neutral-600 dark:text-neutral-400 mb-1">Drop image here or click to upload</p>
              <p className="text-xs text-neutral-500 dark:text-neutral-500">PNG, JPEG, or TIFF. Max 5MB.</p>
            </>
          )}
        </div>
      ) : (
        <>
          <div className="mb-4 space-y-2">
            {extractResult.extracted.employeeName && (
              <p className="text-sm"><span className="font-medium text-neutral-500 dark:text-neutral-400">Employee:</span> {extractResult.extracted.employeeName}</p>
            )}
            ...
            <div className="flex flex-wrap gap-4 items-end">
              ...
              <div className="space-y-1">
                <label className="block text-[10px] font-medium uppercase tracking-wider text-neutral-500 dark:text-neutral-400">Site *</label>
                <select
                  value={siteId}
                  onChange={(e) => { setSiteId(e.target.value); setPostId(""); }}
                  className="input-modern min-w-[180px]"
                >
              ...
              <div className="space-y-1">
                <label className="block text-[10px] font-medium uppercase tracking-wider text-neutral-500 dark:text-neutral-400">Post *</label>
                <select
                  value={postId}
              ...
              <tbody>
                {entries.map((e, i) => (
                  <tr key={i} className="border-b border-neutral-100 dark:border-neutral-800">
                    ...
                    <td className="py-1 px-2 text-neutral-500">{e.hoursWorked ?? "-"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              disabled={importing || !employeeId || !postId || entries.length === 0}
            ...
            <button
              type="button"
              onClick={() => { setExtractResult(null); setEntries([]); setError(""); }}
              className="btn-ghost"
            >
              Cancel
            </button>
