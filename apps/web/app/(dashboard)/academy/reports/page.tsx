  ...
  return (
    <div className="w-full min-w-0 space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-security-navy-600">Operational and compliance analytics with date-range filtering.</p>
      </div>

      {error && <div className="rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700">{error}</div>}

      <div className="rounded-2xl border border-security-navy-100 bg-white p-5 shadow-security-card">
        <h2 className="text-sm font-semibold uppercase tracking-wide text-security-navy-500">Date range</h2>
        <div className="mt-3 flex flex-wrap items-end gap-2">
          <label>
            <span className="mb-1 block text-xs font-medium uppercase tracking-wide text-security-navy-500">From</span>
            <DateInput value={from} onChange={setFrom} className="input-modern" showToday ariaLabel="Report from date" />
          </label>
          <label>
            <span className="mb-1 block text-xs font-medium uppercase tracking-wide text-security-navy-500">To</span>
            <DateInput value={to} onChange={setTo} className="input-modern" showToday ariaLabel="Report to date" />
          </label>
          <button className="rounded-security-lg" onClick={load}>Apply filters</button>
        </div>
      </div>
      ...
      {data ? (
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {Object.entries(data).map(([k, v]) => (
            <div key={k} className="rounded-2xl border border-security-navy-100 bg-white p-4 shadow-security-card">
              <p className="text-[11px] font-semibold uppercase tracking-wide text-security-navy-500">{k}</p>
        ...
        <div className="rounded-2xl border border-security-navy-100 bg-white p-6 text-sm text-security-navy-500 shadow-security-card">
