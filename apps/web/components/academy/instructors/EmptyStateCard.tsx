...
export function EmptyStateCard({
  ...
  return (
    <div className="mx-auto w-full max-w-lg rounded-xl border border-slate-200 bg-slate-50/70 p-6 text-center">
      ...
      <p className="mx-auto mt-2 max-w-md text-sm text-black">{description}</p>
      <div className="mt-4 flex flex-wrap items-center justify-center gap-2">
        <button type="button" onClick={onCta} className="btn-primary text-sm py-2 px-4 rounded-security-lg">
          {ctaLabel}
        </button>
        {secondaryCtaLabel && onSecondaryCta && (
          <button type="button" onClick={onSecondaryCta} className="btn-secondary text-sm py-2 px-4 rounded-lg">
            {secondaryCtaLabel}
          </button>
        )}
      </div>
