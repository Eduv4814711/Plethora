    ...
    <ol className={`space-y-2 sm:space-y-0 sm:flex sm:flex-wrap sm:items-center sm:gap-x-2 sm:gap-y-2 ${className}`}>
      {steps.map((step, i) => (
        <li key={step} className="flex items-center gap-2 sm:gap-2">
          <span className="flex min-w-0 items-center gap-2 rounded-lg border border-neutral-200 bg-white px-3 py-2 text-sm shadow-sm dark:border-neutral-700 dark:bg-neutral-950 sm:border-0 sm:bg-transparent sm:px-0 sm:py-0 sm:shadow-none">
            <span
          ...
          {i < steps.length - 1 && (
            <svg
              className="hidden h-3.5 w-3.5 shrink-0 text-neutral-300 dark:text-neutral-600 sm:block"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
              aria-hidden
