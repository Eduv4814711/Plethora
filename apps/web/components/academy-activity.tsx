...
export function AcademyActivityRow({ item, compact }: { item: AcademyActivityItem; compact?: boolean }) {
  ...
  if (item.link) {
    return (
      <Link
        href={item.link}
        className="block rounded-xl border border-transparent bg-white/60 p-2.5 text-sm transition hover:border-neutral-200"
      >
        {inner}
      </Link>
    );
  }
  return <div className="rounded-xl border border-neutral-200 bg-white/60 p-2.5 text-sm">{inner}</div>;
}
