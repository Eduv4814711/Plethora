...
const UPLOADS_BASE = join(process.cwd(), "uploads", "academy", "instructors");
...
export async function academyInstructorsRoutes(app: FastifyInstance) {
  ...
  app.post("/:id/documents", { preHandler: academyProtect }, async (request, reply) => {
    ...
    const dir = join(UPLOADS_BASE, companyId, id);
    await mkdir(dir, { recursive: true });
    const filepath = join(dir, filename);
    const fileUrl = `academy/instructors/${companyId}/${id}/${filename}`;
